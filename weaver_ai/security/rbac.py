    ...
    try:
        ...
        if "policies" not in str(resolved_path):
            ...
            if "test" not in str(resolved_path) and "tmp" not in str(resolved_path):
                raise ValueError(f"Role file path must be in policies directory: {resolved_path}")
        ...
        raise ValueError(f"Invalid role file path: {e}")
...
def check_access(user: UserContext, scope: str, *, roles_path: Path) -> None:
    ...
    if scope not in user_scopes:
        raise HTTPException(
            status_code=403,
            detail=f"Access denied: missing required scope '{scope}'"
        )
