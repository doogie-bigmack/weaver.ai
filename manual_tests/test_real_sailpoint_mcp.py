...
class LiveSailPointAgent(BaseAgent):
    ...
    async def process(self, event: Event) -> Result:
        ...
        if "list" in query.lower() and "user" in query.lower():
            ...
            result = await self.execute_tool(
                "sailpoint_iiq",
                {"operation": "list_users", "query": {"limit": 5, "offset": 0}},
                check_permissions=False,
            )
        ...
        if "list" in query.lower() and "role" in query.lower():
            ...
            result = await self.execute_tool(
                "sailpoint_iiq",
                {"operation": "list_roles", "query": {"limit": 5, "offset": 0}},
                check_permissions=False,
            )
...
Note whether the data is from a live SailPoint instance or demo data."""
            ...
            try:
                response = await self.model_router.generate(
                    prompt=prompt, model="gpt-4o-mini", max_tokens=500, temperature=0.7
                )
                        ...
                        "sailpoint_operations": [
                            {
                                ...
                                "error": (
                                    op[1].error if op[1] and not op[1].success else None
                                ),
                            }
    ...
    for query in queries:
        ...
        event = Event(
            ...
            metadata=EventMetadata(
                source="test_script", target=agent.name, workflow_id="test_real_mcp"
            ),
        )
                    ...
                    for op in ops:
                        ...
                        if op["success"]:
                            ...
                            if op["data"]:
                                ...
                                if "users" in data:
                                    print(
                                        f"    Total Users: {data['users'].get('total', 'N/A')}"
                                    )
                                if "roles" in data:
                                    print(
                                        f"    Total Roles: {data['roles'].get('total', 'N/A')}"
                                    )
                                if "identities" in data:
                                    print(
                                        f"    Identities Retrieved: {len(data['identities'])}"
                                    )
                                if "bundles" in data:
                                    print(
                                        f"    Bundles Retrieved: {len(data['bundles'])}"
                                    )
                        else:
                            print("    Status: ✗ Failed")
