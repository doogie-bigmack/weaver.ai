    ...
    async def list_identities(self, limit: int = 10, offset: int = 0) -> dict[str, Any]:
        ...
        try:
            response = await self.client.get(
                f"{self.base_url}/identities", params=params
            )
            return response.json()
    ...
    async def execute_tool(
        ...
        try:
            response = await self.client.post(
                f"{self.base_url}/execute/{tool_name}", json=args
            )
            return response.json()
        except Exception as e:
            return {"error": str(e)}
    ...
    try:
        ...
        if "error" in identities:
            ...
            if "503" in str(identities.get("error", "")):
                ...
                print(
                    "  The HTTP server is running but cannot connect to SailPoint at:"
                )
            ...
            if "Resources" in identities:
                # SCIM format
                print(
                    f"  Total Identities: {identities.get('totalResults', 'unknown')}"
                )
                ...
                # Direct format
                print(
                    f"  Total: {identities.get('total', len(identities['identities']))}"
                )
