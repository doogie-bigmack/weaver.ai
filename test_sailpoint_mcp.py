...
async def test_sailpoint_mcp_server():
    ...
    print(f"✅ Identity Governance Agent initialized")
    ...
    if result.success:
        ...
        for user in users[:3]:  # Show first 3
            print(f"   - {user['name']} ({user['email']}) - {user['department']} - {user['status']}")
    else:
    ...
    if result.success:
        ...
        for role in roles[:3]:  # Show first 3
            print(f"   - {role['name']} ({role['type']}) - {role['entitlements']} entitlements")
    else:
            ...
            "content": [
                {
                    ...
                    "text": json.dumps({
                        ...
                        "roles": {"total": 85, "business_roles": 45, "it_roles": 40},
                    }),
                },
            ],
        },
